const SITE_URL =
  (process.env.NEXT_PUBLIC_SITE_URL ?? "https://www.sahneva.com").replace(/\/$/, "");
...
const DEFAULT_RETURN_POLICY = {
  ...
  name: "Etkinlik hizmetlerinde iade bulunmamaktadır.",
  ...
  const productNodes = entry.products.map((product, index) => {
    ...
    const node = {
      ...
      offers: {
        ...
        priceCurrency,
        priceValidUntil: product.priceValidUntil ?? entry.priceValidUntil ?? DEFAULT_PRICE_VALID_UNTIL,
        businessFunction: "https://schema.org/ProvideService",
        eligibleRegion: { "@type": "Country", name: "Türkiye" },
        seller: { "@id": ORGANIZATION_ID },
        itemCondition: "https://schema.org/NewCondition",
        shippingDetails: product.shippingDetails ?? entry.shippingDetails ?? DEFAULT_SHIPPING_DETAILS,
        hasMerchantReturnPolicy:
          product.hasMerchantReturnPolicy ?? entry.hasMerchantReturnPolicy ?? DEFAULT_RETURN_POLICY,
      },
  ...
  const aggregateOffer =
    prices.length > 0
      ? {
          ...
          availability: "https://schema.org/InStock",
          url: `${pageUrl}#teklif`,
          shippingDetails: entry.shippingDetails ?? DEFAULT_SHIPPING_DETAILS,
          hasMerchantReturnPolicy: entry.hasMerchantReturnPolicy ?? DEFAULT_RETURN_POLICY,
          price: (
  ...
  const baseOfferNodes = baseOfferEntries
    .map((offer, index) => {
      ...
      return {
        ...
        seller: { "@id": ORGANIZATION_ID },
        itemCondition: "https://schema.org/NewCondition",
        shippingDetails: offer.shippingDetails ?? entry.shippingDetails ?? DEFAULT_SHIPPING_DETAILS,
        hasMerchantReturnPolicy:
          offer.hasMerchantReturnPolicy ?? entry.hasMerchantReturnPolicy ?? DEFAULT_RETURN_POLICY,
        itemOffered: {
          "@type": "Service",
          "@id": serviceId,
